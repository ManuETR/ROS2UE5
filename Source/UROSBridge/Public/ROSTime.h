...
public:
	...
	{
		uint32 Secs = (uint32)(JsonObject->GetNumberField("sec"));
		uint32 NSecs = (uint32)(JsonObject->GetNumberField("nanosec"));
		return FROSTime(Secs, NSecs);
	}

	FString ToString() const
	{
		return TEXT("Time { sec = ") + FString::FromInt(Secs) + TEXT(", nanosec = ") + FString::FromInt(NSecs) + TEXT(" }");
	}
